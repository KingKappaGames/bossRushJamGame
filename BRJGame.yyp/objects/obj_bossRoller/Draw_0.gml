if(irandom(7) == 0) {
	...
	draw_set_alpha(.24);
	draw_ellipse_color(x + room_width * .25 - 34 + irandom_range(-16, 16), y - 22 + irandom_range(-8, 8), x + room_width * .25 + 34 + irandom_range(-16, 16), y + 13 + irandom_range(-8, 8), #BA9168, #dAb198, false);
	draw_set_alpha(1);
...
var _thighSpriteWidth = sprite_get_width(spr_rollerThigh);
var _footSpriteHeight = sprite_get_width(spr_rollerFoot);
...
for(var _legI = 0; _legI < 14; _legI++) {
	...
	if(_legDrawI < legPairsOnGround * 2) {
		...
		if(_legOrigin[1] < _legPos[1]) { // if foot below, on screen, base draw foot second
			draw_sprite_ext(spr_rollerThigh, 0, _legOrigin[0], _legOrigin[1], _kneeDist / _thighSpriteWidth, 1, _kneeDir, _healthColor, 1);
			draw_sprite_ext(spr_rollerFoot, 0, _legMidX, _legMidY, _footFacingDir, _footDist / _footSpriteHeight, _footDir + 90, _healthColor, 1);
...
draw_sprite_ext(sprite_index, image_index, x, y, image_xscale * directionFacing, image_yscale, image_angle, _healthColor, 1);
